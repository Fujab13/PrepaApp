...
- `styles/global.css` — multi-theme system via CSS custom properties on `:root` (`--bg`, `--surface`, `--surface2`, `--text`, `--text-muted`, `--border`, `--border-strong`, `--overlay-sutil`, `--correct`, `--wrong`, plus optional per-theme extras like `--sidenav-bg`); a purchasable/free theme (see `data/storeItems.js` `COIN_ITEMS`, `temaId`) sets `document.documentElement.dataset.tema` (`utils/temas.js`), matched by a `:root[data-tema="..."]` block that only needs to override the variables it wants to differ. **New UI must use these variables for any neutral surface/text/border color — never hardcode a gray/white/black.** Accent/brand colors (`materia.color`, the `#7c5cbf` primary-button purple, `--correct`/`--wrong`) are the deliberate exception and stay fixed across themes. Adding a new theme = one new `:root[data-tema="..."]` block in this file; full rationale is documented at the top of the file itself.
